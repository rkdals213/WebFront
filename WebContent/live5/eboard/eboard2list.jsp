...
<script>
		...
		$.get(
			...
			function(data, status){
				$.each(data, function(index, dto){
					...
					+"</td><td><a href='javascript:brd_detail("+dto.articleno+");'"+dto.subject
					+"</a></td><td>"+dto.userid
					+"</td><td>"+dto.regtime+"</td></tr>";
					$("#brd_list.tbody").append(str);
				});
			},
			"json"
		);
	...
	function brd_detail(num){
		$.get(
			"${pageContext.request.contextPath}/EboardController",
			{command:'brd_datail', articleno:num},
			function(data, status){
				$.each(data, function(index, dto){
					$("#mbr_id").val(dto.userid);
					$("#brd_title").val(dto.subject);
					$("#brd_cnts").val(dto.content);
